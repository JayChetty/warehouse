...
describe AlphaNumericRackTranslator do
  let(:item_finder){ double('finder') }
  let(:adapter){adapter = AlphaNumericRackTranslator.new(['a','b','c'], 10 )}
  ...
  it "should alter number of reversed racks if stated" do
    adapter_with_inversion = AlphaNumericRackTranslator.new( ['a','b','c'], 10, [ 'a','c' ] )
  ...
  it "raise exeception if rack key is out of range" do
    expect{ adapter.key_to_index("d1") }.to raise_error( AlphaNumericRackTranslator::KeyOutOfRangeError )
  end

  it "raise exception if rack key is out of range" do
    expect{ adapter.key_to_index("a0") }.to raise_error( AlphaNumericRackTranslator::KeyOutOfRangeError )
  end
end